...
ok('it names the billing once', (line.match(/RSR-DW-082026-001/g) || []).length === 1,
   String((line.match(/RSR-DW-082026-001/g) || []).length));
...
el('kDate').value = '2026-08-21'; el('kRate').value = '3000'; el('kType').value = 'DC';
app.openCat('DC');
await el('kCommit').onclick();
ok('the catalog picker confirms', /class="made"/.test(el('mkRecent').innerHTML),
   el('mkRecent').innerHTML);
ok('naming its billing', /RSR-DC-082026-001/.test(el('mkRecent').innerHTML));
