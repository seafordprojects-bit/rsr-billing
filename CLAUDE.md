...
Everything is one file — `index.html` — plus two Supabase Edge Functions. Open
the file in a browser and it works; there is no build step, no bundler, no
framework. The one dev dependency, pglite, exists for `test/rpc.test.mjs`
alone (`npm install` once; nothing in the app needs it).
...
**One client, one row is enforced by `name_canon`, not by `name`.** `name` is
raw text, so `Seaford` and `seaford  ` both satisfied its unique constraint and
the second insert simply *succeeded* — no 409, no heal, two rows. `clientRec`
then returned whichever the server's `order=name.asc` happened to put first,
and everything a send reads off the client — Bill To, the letter's salutation,
the cc — came from that row, while an address corrected on the other was never
used again. Nothing surfaced it either: the client picker dedupes canonically
before it suggests, so two rows show as one entry, and only Settings → Clients
lists both.

`name_canon` is a **generated** column — `lower(btrim(regexp_replace(name,
'\s+', ' ', 'g')))`, exactly what `canonClient` compares on — and the unique
index is on it. Generated rather than written, so no existing spelling is
rewritten and a billing still matches the client name it was issued under. The
...
job dies in Pending writes — the index without the lookup is a stuck queue in
place of a silent duplicate. For the same reason the heal **never compares
`name`**: the row was found *by* the canonical form of it, so a differing
spelling is what matched, not what is contested, and the server's spelling is
the one its billings already carry. Without that skip every case variant
refuses itself with "both changed name".

That canonical form is currently written out inline in `clientRec`,
`canonClient` and `clientSuggest`, and once as `cliKey` for the heal. Four
copies of one rule, and the SQL expression is a fifth — change one and the
...
`cfg.autoMarkPrint`, `cfg.terms`, `cfg.rate`, `cfg.lockMins`, `cfg.lockCode`
and the company details are **per-device** — they are not in
`pushSharedSettings`, which carries types, payment, the billing counters and
the letter. The two lock keys are per-device for different reasons: the window
is a preference, and the passcode must never leave the device at all.
...
documents to the client. The function checks `x-drydock-secret` against
`DRYDOCK_INGRESS_SECRET` (constant-time compare; an UNSET secret answers 500,
never lets anything through), rebuilds the payload from scratch (only the
fifteen known keys, header bytes turned into a space the way send-statement
does), and calls `receive_drydock_job(jsonb)` with the service key.

That RPC, in one transaction: inserts the receipt with `on conflict
(dispatch_id) do nothing` — a retry gets `created:false` and the existing
group, so a client can never be billed twice for one send; fills the client
row where blank and never overwrites it; takes an advisory lock on the month
prefix and mints `RSR-DC-MMYYYY-###` by the same max-scan `nextCode` runs on a
...
failure 500 so it is retried later. **`verify_jwt = false`** for this function
in `supabase/config.toml` — the caller holds a secret, not a session.
Forgetting it makes every drydocking send sit at `emailed_billing_pending`
over there with "billing ingress returned 401".

In Monitoring, `receiptMap` (refreshed beside `sendMap`) puts a `From
drydocking` badge and an "Emailed <Manila day> · <who confirmed>" meta on the
card. Keyed on the receipt, not the DC type, so a hand-entered Drydocking Cert
billing stays unbadged. Neither reaches the statement or the email: `drydock`
...
A passcode overlay on an idle timer, **off unless a device turns it on**
(`cfg.lockMins` minutes and `cfg.lockCode`, both required — a window with no
passcode would lock a device with no way back in). The shared PC is unaffected
without anyone opting out, and no timer runs at all while it is off.

**Deliberately not `visibilitychange`.** A desktop tab switch and a phone going
to sleep are the same event carrying no reason, so the shared PC would have
demanded a passcode every time somebody glanced at another tab. A timer can at
least be told what idleness means. The trade is that a phone locked in a pocket
is not locked in the app until the window elapses.

**An overlay, never a reload.** `pendingSend`, the letter half-composed in
`lBody`, an open sheet and the queue are all exactly where they were. None of
that is persisted — `persist()` writes rows, queue, clients, catalog and cfg,
not sheet state — so a lock that reloaded would silently destroy a send in
...
single throttle row, so mistyped lock codes would lock out unbilling firm-wide
— and a phone waking with no signal could not get back in. What the local check
buys is worth being exact about: it stops someone picking up an unattended
handset. The passcode is plain text in that device's `localStorage`, so it does
not stop someone holding the device with time and a laptop. There is no
wrong-attempt limit **on purpose** — a lockout strands the phone in the field.

#### The layers, and who owns the body scroll

Stacking, low to high: `.fab-bar` 35, `header.top` 40, `.scrim` 50, `.sheet`
51, `.toast` 70, `#dropOverlay` 80, `#gate` 90, **`#lock` 95**. The lock is
above the gate because it is about who is holding the device, and it has to
cover the sign-in screen too. One consequence: if the session expires while
locked, `sb()` raises the gate *underneath*, so unlocking lands on sign-in.
Two prompts in a row is correct, not a bug.

`#lock` toggles `display`, never `opacity` — a closed overlay left at
`opacity:0` ate clicks across the whole window once already, which is why
`.sheet` carries `pointer-events:none`. It is also in the `@media print`
`display:none !important` list; left out, it prints on top of the billing, and
nothing but section 12 of `MANUAL-TEST.md` would ever catch that.

**`scrollLock()` is the only writer of `document.body.style.overflow`.**
It derives the value from `locked || gateOpen || openSheet`. There were five
direct writers — `showGate`, `hideGate`, the gate's Connection-settings button,
`show()` and `hide()` — and each cleared it flat, so closing a sheet under a
raised lock would have let the page scroll behind it. Add a fourth thing that
wants the body held and it goes in that expression, not in a new assignment.
...
> every statement after it — the same trap as the unbill-log policy. It sits in
> a `do` block that names the duplicates and raises a warning instead, so the
> rest of the script still runs. The consequence is real: **on a project that
